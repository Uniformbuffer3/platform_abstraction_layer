            ...
            for seat in self.environment.get_all_seats() {
                ...
                smithay_client_toolkit::seat::with_seat_data(&seat, |seat_data| {
                    ...
                    if !seat_data.defunct {
                        ...
                        let event = SeatEvent::SeatAdded((id, seat_data.clone()).into());
            ...
            self.event_listeners.seat = Some(self.environment.listen_for_seats(
                |seat, data, mut dispatch_data| {
                    ...
                    if data.defunct {
                        ...
                        let event = SeatEvent::SeatRemoved;
                        ...
                        match (current_data.has_pointer, data.has_pointer) {
                            (false, true) => {
                                let event = SeatEvent::SeatChanged(SeatCapability::PointerAdded);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            (true, false) => {
                                let event = SeatEvent::SeatChanged(SeatCapability::PointerRemoved);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            _ => (),
                        }
                        match (current_data.has_keyboard, data.has_keyboard) {
                            (false, true) => {
                                ...
                                handle_keyboard(id, keyboard);
                                let event = SeatEvent::SeatChanged(SeatCapability::KeyboardAdded);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            (true, false) => {
                                let event = SeatEvent::SeatChanged(SeatCapability::KeyboardRemoved);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            _ => (),
                        }
                        match (current_data.has_touch, data.has_touch) {
                            (false, true) => {
                                let event = SeatEvent::SeatChanged(SeatCapability::TouchAdded);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            (true, false) => {
                                let event = SeatEvent::SeatChanged(SeatCapability::TouchRemoved);
                                dispatch_context.events.push(Event::Seat { id, event });
                            }
                            _ => (),
                        }
...
impl crate::definitions::InputBackend for WaylandPlatform {
    ...
    fn keyboard_layout(&self) -> String {
        self.dispatch_context.keystroke_decoder.layout().clone()
    }
...
impl crate::definitions::GraphicBackend for WaylandPlatform {
    fn create_surface(
        &mut self,
                    ...
                    match event {
                        smithay_client_toolkit::window::Event::Configure {
                            ...
                            if let Some(size) = new_size {
                                let event = SurfaceEvent::Resized{width: size.0, height: size.1};
                                let id = surface_id;
                                dispatch_data
                                    .get::<DispatchContext>()
                                    .unwrap()
                            ...
                            /*
                                                states.iter().for_each(|state|{
                                                    ...
                                                    dispatch_data.get::<super::DispatchContext>().unwrap().events.push(SurfaceEventInner::ChangedTiling.into())
                                                })
...
/*
    ...
    fn raw_surface_handle(&self, _surface: crate::definitions::SurfaceId) -> crate::definitions::RawSurfaceHandle {
        unimplemented!();
        //RawSurfaceHandle::Xcb(raw_window_handle::unix::XcbHandle {window: surface.into(),connection: self.connection, ..XcbHandle::empty()})
    }
/*
